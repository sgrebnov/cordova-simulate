var CameraPlugin = (function () {
    ...
    function handleTakePicture(success, fail, service, action, args) {
        ...
        if (document.getElementById('camera-host').checked) {
            ...
            var url = (URL || webkitURL).createObjectURL(document.getElementById('camera-filename').files[0]);
            success(url);
        }
    }
    ...
    function updateDialogImage() {
        var url = (URL || webkitURL).createObjectURL(document.getElementById('camera-dialog-filename').files[0]);
